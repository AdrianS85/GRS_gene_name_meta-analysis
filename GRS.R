...
# test_filters <- biomaRt::listFilters(mart = test_mart)
# 
...
annotations_Probe_ID <- master_annotator_for_known_identfiers(descriptions_ = descriptions, str_filename_ = 'input_ProbeID.tsv', str_identifier_type__ = identifiers_used_for_annotation_global, str_experiment_name = 'Probe_ID') # 195495 vs 195495
...
pre_annotations_gemma_probeID <- get_gemma_annotations_for_data(str_filename_ = 'checked_input/input_Gemma.tsv', list_gemma_platforms = gemma_platforms) # 2002 vs 2002
...
pre_annotations_gemma_probeID$Probe_ID <- pre_annotations_gemma_probeID$external_gene_name
pre_annotations_gemma_probeID$external_gene_name <- NULL

# We need to select only rows that actually have gene names, and put rest where it belong
second_stage_input_gemma_probeID_gene_names <- subset(x = pre_annotations_gemma_probeID, subset = !is.na(pre_annotations_gemma_probeID$Probe_ID)) ### !!! In this data, for every probeID gene name was produced by gemma database. Now this needs to be passed through ncbi to get entrezID, which then will be changed into final gene names by ensembl
second_stage_leftovers_input_gemma_probeID <- subset(x = pre_annotations_gemma_probeID, subset = is.na(pre_annotations_gemma_probeID$Probe_ID)) ### !!! This is leftovers

rm(pre_annotations_gemma_probeID)

annotated_second_stage_input_gemma_probeID_gene_names <- annotate_identifiers_to_geneID_wrapper_for_using_dfs_as_input(descriptions__ = descriptions, df_to_annotate = second_stage_input_gemma_probeID_gene_names, str_experiment_name__ = 'second_stage_leftovers_input_gemma_probeID')
...
annotations_ensembl_gene_id <- master_annotator_for_known_identfiers(descriptions_ = descriptions, str_filename_ = 'checked_input/input_EnsemblGeneId.tsv', str_identifier_type__ = 'ensembl_gene_id') #15302 vs 15302

annotations_ensembl_transcript_id <- master_annotator_for_known_identfiers(descriptions_ = descriptions, str_filename_ = 'checked_input/input_EnsemblTranscriptId.tsv', str_identifier_type__ = 'ensembl_transcript_id') #2419 vs 2419

annotations_entrezgene_id <- master_annotator_for_known_identfiers(descriptions_ = descriptions, str_filename_ = 'checked_input/input_EntrezGeneId.tsv', str_identifier_type__ = 'entrezgene_id') #29459 vs 29459

annotations_refseq_mrna <- master_annotator_for_known_identfiers(descriptions_ = descriptions, str_filename_ = 'checked_input/input_RefSeqMRNA.tsv', str_identifier_type__ = 'refseq_mrna') #4715 vs 4715

annotations_mgi <- master_annotator_for_known_identfiers(descriptions_ = descriptions, str_filename_ = 'checked_input/input_mgi_symbol.tsv', str_identifier_type__ = 'mgi_symbol') #250 vs 250

annotations_accession <- master_annotator_for_known_identfiers(descriptions_ = descriptions, str_filename_ = 'checked_input/input_accession.tsv', str_identifier_type__ = 'embl') #3421 vs 3421
...
annotations_known_identifiers <- rbind(annotations_ensembl_gene_id, annotations_ensembl_transcript_id, annotations_entrezgene_id, annotations_refseq_mrna, annotations_mgi, annotations_accession)

finalized_known_identifiers <- subset(x = annotations_known_identifiers, subset = !is.na(annotations_known_identifiers$external_gene_name))
leftover_known_identifiers <- subset(x = annotations_known_identifiers, subset = is.na(annotations_known_identifiers$external_gene_name))

check_was_the_spliting_of_df_by_filtering_ok(df_original = annotations_known_identifiers, list_df_splited = list(finalized_known_identifiers, leftover_known_identifiers))
####### SECOND ORDER ANNOTATION ####### 
#################################
####### KNOWN IDENTIFIERS ####### 
#################################
...
#########################################
### !!! BEWARE!!! LOC OR OTHER SUCH IDENTIFIERS MAY BE MISINTERPRETED BY ENTREZ AS GENEIDS! THIS MAY INDUCE SOME SERIOUS ISSUES!!!!!!
...
annotated_other_ids_to_geneIDs <- annotate_identifiers_to_geneID(str_filename_ = 'checked_input/input_RGD_GM_RNA_7SK.tsv', str_experiment_name = 'other_ids', descriptions_ = descriptions, chr_gene_identifier_ = 'Gene Name') #515 vs
# OR
# annotated_other_ids_to_geneIDs <- readr::read_tsv(file = 'other_id/data_v4_for_ncbi_output_of_ncbi_query.tsv')

annotated_names_to_geneIDs <- annotate_identifiers_to_geneID(str_filename_ = 'checked_input/input_gene_symbol.tsv', str_experiment_name = 'names', descriptions_ = descriptions, bool_reformat_names = F, chr_gene_identifier_ = 'Gene Name') #4433 vs 
####### NCBI ANNOTATION ####### 
