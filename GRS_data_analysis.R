...
####### PREPARING SUBSET TABLES FOR ACUTE-CHRONIC ####### 
####### PREPARING SUBSET TABLES FOR SEX #######
### !!! Beware, this is not good. Experiment 27_1 needs to be removed and exp 16_1 need to be substituted for roszkowski wydzielone. I did it later in the script for analysis of all, and not 3+publication genes. Thus, this is obsolete
search_for <- list('male' = 'male', 'female' = 'female')

experiments_to_subset <- lapply(X = search_for, function(x){
  descriptions_1_and_2 %>%
    dplyr::select(Group_ID, Gender_clean) %>%
    dplyr::filter(descriptions_1_and_2$Gender_clean == x)
})
...
search_for <- list('male_all' = 'male', 'female_all' = 'female')
...
female_all <- dplyr::select(.data = female_all, -`16_1`, -`27_1`)
...
roszkowski_add <- readr::read_tsv(file = 'sex/roszkowski wydzielone.txt')

female_all <- merge(x = female_all, y = roszkowski_add, by = 'lower_final_gene_name', all = T)

female_all$`16_1`[is.na(female_all$`16_1`)] <- 0


objects_ <- list('male_all' = male_all, 'female_all' = female_all)
...
perc4_analysis <- list('dir' = '4_percent', 'gene_name_col_name' = 'lower_final_gene_name')

perc4_analysis$compare_input <- read_excel_all_sheets(directory = perc4_analysis$dir, file_name = 'dane porownanwcze.xlsx', colNames_ = F)

perc4_analysis$compare_input_prepared <- perc4_analysis_input_prep_wrapper(compare_input_ = perc4_analysis$compare_input, gene_name_col_name_ = perc4_analysis$gene_name_col_name)



perc4_analysis$top_4_perc_input <- read_excel_all_sheets(directory = perc4_analysis$dir, file_name = '/A top stress genes top 4 %.xlsx', colNames_ = T)
perc4_analysis$top_4_perc_input[[1]] <- perc4_analysis$top_4_perc_input[[1]][-c(1,2)]
perc4_analysis$top_4_perc_input[[2]] <- perc4_analysis$top_4_perc_input[[2]][-c(1,2)]

perc4_analysis$top_4_perc_input[[1]]$perc_of_upregulated <- as.numeric(perc4_analysis$top_4_perc_input[[1]]$perc_of_upregulated)

perc4_analysis$top_4_perc_input[[2]]$fraction_of_upregulated <- as.numeric(perc4_analysis$top_4_perc_input[[2]]$fraction_of_upregulated)



perc4_analysis$merged_output <- merge(x = perc4_analysis$top_4_perc_input$`paper x fraction rank`, y = perc4_analysis$compare_input_prepared, by = perc4_analysis$gene_name_col_name, all.x = T)

readr::write_excel_csv2(perc4_analysis$merged_output, paste0(perc4_analysis$dir, '/paper_x_faction_rank_and_dane_porwownawcze_perc_4.xlsx'))

perc4_analysis$merged_output_2 <- merge(x = perc4_analysis$top_4_perc_input$`peper rank`, y = perc4_analysis$compare_input_prepared, by = perc4_analysis$gene_name_col_name, all.x = T)

readr::write_excel_csv2(perc4_analysis$merged_output_2, paste0(perc4_analysis$dir, '/paper_rank_and_dane_porwownawcze_perc_4.xlsx'))



perc4_analysis$all_genes <- openxlsx::read.xlsx(xlsxFile = paste0(perc4_analysis$dir, '/All stress genes (1).xlsx'), sheet = 1)

perc4_analysis$merged_output_3 <- merge(x = perc4_analysis$all_genes, y = perc4_analysis$compare_input_prepared, by = perc4_analysis$gene_name_col_name, all.x = T)

readr::write_excel_csv2(perc4_analysis$merged_output_3, paste0(perc4_analysis$dir, '/all_genes_and_dane_porwownawcze_perc_4.xlsx'))



perc4_analysis$GC_res_IEG <- openxlsx::read.xlsx(xlsxFile = paste0(perc4_analysis$dir, '/GC-responsive & IEG cluster.xlsx'), sheet = 1, colNames = F)
colnames(perc4_analysis$GC_res_IEG) <- 'lower_final_gene_name'
perc4_analysis$GC_res_IEG$GC_res_IEG <- 'GC_res_IEG'

perc4_analysis$merged_output_4 <- merge(x = perc4_analysis$GC_res_IEG, y = perc4_analysis$compare_input_prepared, by = perc4_analysis$gene_name_col_name, all.x = T)

readr::write_excel_csv2(perc4_analysis$merged_output_4, paste0(perc4_analysis$dir, '/GC_res_IEG_and_dane_porwownawcze_perc_4.xlsx'))



perc4_analysis$acute_top_50_100 <- read_excel_all_sheets(directory = perc4_analysis$dir, file_name = '/acute top 50 & 100.xlsx', colNames_ = T)

perc4_analysis$acute_top_50_100[[1]]$`fraction_of_up-regulated` <- as.numeric(perc4_analysis$acute_top_50_100[[1]]$`fraction_of_up-regulated`)

perc4_analysis$acute_top_50_100[[2]]$`fraction_of_up-regulated` <- as.numeric(perc4_analysis$acute_top_50_100[[2]]$`fraction_of_up-regulated`)


perc4_analysis$merged_output_5 <- merge(x = perc4_analysis$acute_top_50_100$`acute top 50`, y = perc4_analysis$compare_input_prepared, by = perc4_analysis$gene_name_col_name, all.x = T)

readr::write_excel_csv2(perc4_analysis$merged_output_5, paste0(perc4_analysis$dir, '/acute_top_50_and_dane_porwownawcze_perc_4.xlsx'))

perc4_analysis$merged_output_6 <- merge(x = perc4_analysis$acute_top_50_100$`acute top 100`, y = perc4_analysis$compare_input_prepared, by = perc4_analysis$gene_name_col_name, all.x = T)

readr::write_excel_csv2(perc4_analysis$merged_output_6, paste0(perc4_analysis$dir, '/acute_top_100_and_dane_porwownawcze_perc_4.xlsx'))

perc4_analysis$prolonged <- read_excel_all_sheets(directory = perc4_analysis$dir, file_name = '/prolonged comparison.xlsx', colNames_ = T)
perc4_analysis$prolonged$prolonged$`fraction_of_up-regulated` <- as.numeric(perc4_analysis$prolonged$prolonged$`fraction_of_up-regulated`)

perc4_analysis$merged_output_7 <- merge(x = perc4_analysis$prolonged$prolonged, y = perc4_analysis$compare_input_prepared, by = perc4_analysis$gene_name_col_name, all.x = T)


readr::write_excel_csv2(perc4_analysis$merged_output_7, paste0(perc4_analysis$dir, '/prolonged_and_dane_porwownawcze_perc_4.xlsx'))

write.table(x = perc4_analysis$merged_output_7, file = paste0(perc4_analysis$dir, '/prolonged_and_dane_porwownawcze_perc_4.tsv'), sep = '\t', dec = ',', row.names = F)
...
### VUL VS RES ANALYSIS ###
vul_res_analysis <- list('dir' = 'vul_vs_res')

vul_res_analysis$input <- read_excel_all_sheets(directory = vul_res_analysis$dir, file_name = 'all_exp_and_paper_numbers_v2 25 maj vulnerable vs resilient.xlsx', colNames_ = T)

colnames(vul_res_analysis$input$ExpNbPerc_resilient_all) <- c('lower_final_gene_name', purrr::map_chr(.x = colnames(vul_res_analysis$input$ExpNbPerc_resilient_all)[-1], .f = function(x) {paste0(x, '_res')}))
colnames(vul_res_analysis$input$ExpNbPerc_vulnerable_all) <- c('lower_final_gene_name', purrr::map_chr(.x = colnames(vul_res_analysis$input$ExpNbPerc_vulnerable_all)[-1], .f = function(x) {paste0(x, '_vul')}))

vul_res_analysis$merged <- merge(x = vul_res_analysis$input$ExpNbPerc_resilient_all, y = vul_res_analysis$input$ExpNbPerc_vulnerable_all, by = 'lower_final_gene_name', all = T)


  
vul_res_analysis$vul_only_genes_subset <- ifelse(
  test = !is.na(vul_res_analysis$merged$no_of_exps_vul) & is.na(vul_res_analysis$merged$no_of_exps_res), 
  yes = T, 
  no = F)

vul_res_analysis$vul_only_genes <- subset(x = vul_res_analysis$merged, subset = vul_res_analysis$vul_only_genes_subset)



vul_res_analysis$res_only_genes_subset <- ifelse(
  test = !is.na(vul_res_analysis$merged$no_of_exps_res) & is.na(vul_res_analysis$merged$no_of_exps_vul), 
  yes = T, 
  no = F)

vul_res_analysis$res_only_genes <- subset(x = vul_res_analysis$merged, subset = vul_res_analysis$res_only_genes_subset)



vul_res_analysis$opposite_direction_genes_subset <- ifelse(
  test = (vul_res_analysis$merged$perc_of_upregulated_res > 0.5 & vul_res_analysis$merged$perc_of_upregulated_vul < 0.5) | (vul_res_analysis$merged$perc_of_upregulated_res < 0.5 & vul_res_analysis$merged$perc_of_upregulated_vul > 0.5), 
  yes = T, 
  no = F)

vul_res_analysis$opposite_direction_genes <- subset(x = vul_res_analysis$merged, subset = vul_res_analysis$opposite_direction_genes_subset)



openxlsx::write.xlsx(x = list(vul_res_analysis$vul_only_genes, vul_res_analysis$res_only_genes, vul_res_analysis$opposite_direction_genes), file = paste0(vul_res_analysis$dir, '/vul_vs_res_analysis.xlsx'), sheetName = c('vul_only_genes', 'res_only_genes', 'opposite_direction_genes'))
### VUL VS RES ANALYSIS ###
...
### ALL VS CLU ANALYSIS ###
all_clu_analysis <- list('dir' = 'all_vs_clu')

all_clu_analysis$all <- read_excel_all_sheets(directory = all_clu_analysis$dir, file_name = 'All stress genes.xlsx', colNames_ = T)

all_clu_analysis$all$`directional rank all data` <- all_clu_analysis$all$`directional rank all data`[,-1]

all_clu_analysis$all$`directional rank all data`$fraction_of_upregulated <- as.numeric(all_clu_analysis$all$`directional rank all data`$fraction_of_upregulated)

all_clu_analysis$all$`directional rank all data`$lower_final_gene_name <- stringr::str_trim(string = tolower(all_clu_analysis$all$`directional rank all data`$lower_final_gene_name), side = 'both')

all_clu_analysis$clu <- read_excel_all_sheets(directory = all_clu_analysis$dir, file_name = 'klastry do por?wnania.xlsx', colNames_ = F)

all_clu_analysis$clu <- purrr::map2(
  .x = all_clu_analysis$clu, 
  .y = names(all_clu_analysis$clu),
  .f = function(x,y) {
    colnames(x) <- 'lower_final_gene_name'
    
    x[[y]] <- T
    
    x$lower_final_gene_name <- stringr::str_trim(string = tolower(x$lower_final_gene_name), side = 'both')
    
    return(x)
    } )

all_clu_analysis$merged_clu <- purrr::reduce(.x = all_clu_analysis$clu, .f = dplyr::full_join, by = "lower_final_gene_name")

all_clu_analysis$merged_all_clu <- merge(x = all_clu_analysis$all$`directional rank all data`, y = all_clu_analysis$merged_clu, by = 'lower_final_gene_name', all = T)

openxlsx::write.xlsx(x = all_clu_analysis$merged_all_clu, file = paste0(all_clu_analysis$dir, '/all_vs_clu_analysis.xlsx'))
### ALL VS CLU ANALYSIS ###


### SEX COMPARISON ANALYSIS ###
sex_analysis <- list('dir' = 'sex')


sex_analysis$input <- list(
  'male' = merge(paper_number_male_all, exp_number_and_percentage_male_all, by = 'lower_final_gene_name'), 
  'female' = merge(paper_number_female_all, exp_number_and_percentage_female_all, by = 'lower_final_gene_name'))

# Get only those genes that were present in 3 papers or more
sex_analysis$input$female <- subset(x = sex_analysis$input$female, subset = sex_analysis$input$female$number >= 3) 


colnames(sex_analysis$input$male) <- c('lower_final_gene_name', purrr::map_chr(.x = colnames(sex_analysis$input$male)[-1], .f = function(x) {paste0(x, '_m')}))

colnames(sex_analysis$input$female) <- c('lower_final_gene_name', purrr::map_chr(.x = colnames(sex_analysis$input$female)[-1], .f = function(x) {paste0(x, '_f')}))


sex_analysis$merged <- merge(x = sex_analysis$input$male, y = sex_analysis$input$female, by = 'lower_final_gene_name')

sex_analysis$opposite_direction_genes_subset <- ifelse(
  test = (sex_analysis$merged$perc_of_upregulated_f > 0.5 & sex_analysis$merged$perc_of_upregulated_m < 0.5) | (sex_analysis$merged$perc_of_upregulated_f < 0.5 & sex_analysis$merged$perc_of_upregulated_m > 0.5), 
  yes = T, 
  no = F)

sex_analysis$opposite_direction_genes <- subset(x = sex_analysis$merged, subset = sex_analysis$opposite_direction_genes_subset)

sex_analysis$stable_direction_genes <- subset(x = sex_analysis$merged, subset = !sex_analysis$opposite_direction_genes_subset)

sex_analysis$merge_all_f <- merge(x = sex_analysis$input$male, y = sex_analysis$input$female, by = 'lower_final_gene_name', all.y = T)

sex_analysis$f_only_genes_subset <- ifelse(
  test = !is.na(sex_analysis$merge_all_f$no_of_exps_f) & is.na(sex_analysis$merge_all_f$no_of_exps_m), 
  yes = T, 
  no = F)

sex_analysis$f_only_genes <- subset(x = sex_analysis$merge_all_f, subset = sex_analysis$f_only_genes_subset)



openxlsx::write.xlsx(x = list(sex_analysis$merged, sex_analysis$opposite_direction_genes, sex_analysis$stable_direction_genes, sex_analysis$f_only_genes), file = paste0(sex_analysis$dir, '/sex_analysis.xlsx'), sheetName = c('merged', 'opposite_direction_genes', 'stable_direction_genes', 'f_only_genes'))
### SEX COMPARISON ANALYSIS ###



### NEURO-NATURE COMPARISON ANALYSIS ###
neuro_analysis <- list('dir' = 'neuro_nature')

neuro_analysis$inputs <- purrr::map(
  .x = list.files(path = neuro_analysis$dir, pattern = '.*tsv'), 
  .f = function(file_){
    readr::read_tsv(file = paste0(neuro_analysis$dir, '/', file_))
})

neuro_analysis$input <- purrr::reduce(
  .x = neuro_analysis$inputs, 
  .f = dplyr::full_join, 
  by = "Gene")

neuro_analysis$input$Gene <- tolower(neuro_analysis$input$Gene)



temp_biomart <- biomaRt::useMart("ENSEMBL_MART_ENSEMBL", dataset = "mmusculus_gene_ensembl")
neuro_analysis$bm_results <- biomaRt::getBM(
  attributes = "external_gene_name",
  filters = "external_gene_name",
  values = neuro_analysis$input$Gene,
  uniqueRows = T,
  mart = temp_biomart
)
neuro_analysis$bm_results$Gene <- tolower(neuro_analysis$bm_results$external_gene_name)
neuro_analysis$input_bm <- merge(neuro_analysis$input, neuro_analysis$bm_results, by = 'Gene', all.x = T)

neuro_analysis$missing <- neuro_analysis$input_bm$Gene[is.na(neuro_analysis$input_bm$external_gene_name)]

neuro_analysis$input_bm$external_gene_name <- tolower(neuro_analysis$input_bm$external_gene_name)

neuro_analysis$input_bm$external_gene_name[neuro_analysis$input_bm$Gene == 'cyr61'] <- 'ccn1'
neuro_analysis$input_bm$external_gene_name[neuro_analysis$input_bm$Gene == 'fam134a'] <- 'retreg2'
neuro_analysis$input_bm$external_gene_name[neuro_analysis$input_bm$Gene == 'gm15487'] <- 'atp6v0c-ps2'
neuro_analysis$input_bm$external_gene_name[neuro_analysis$input_bm$Gene == 'gm3608'] <- 'capza1-ps1'
neuro_analysis$input_bm$external_gene_name[neuro_analysis$input_bm$Gene == 'kank'] <- 'kank3'
neuro_analysis$input_bm$external_gene_name[neuro_analysis$input_bm$Gene == 'skiv2l2'] <- 'mtrex'
neuro_analysis$input_bm$external_gene_name[neuro_analysis$input_bm$Gene == 'slcza1'] <- 'slc2a1'
neuro_analysis$input_bm$external_gene_name[neuro_analysis$input_bm$Gene == 'srsfb'] <- 'srsfb'
neuro_analysis$input_bm$external_gene_name[neuro_analysis$input_bm$Gene == 'tmtm30a'] <- 'tmem30a'

neuro_analysis$input_bm$gene_name_in_publication <- neuro_analysis$input_bm$Gene
neuro_analysis$input_bm$Gene <- NULL

neuro_analysis$exp_ <- readr::read_tsv('exp_and_paper_numbers/exp_number_and_percentage_.tsv')
neuro_analysis$pap_ <- readr::read_tsv('exp_and_paper_numbers/paper_number_.tsv')



neuro_analysis$input_and_experiements <- merge(x = neuro_analysis$input_bm, y = neuro_analysis$exp_, by.x = 'external_gene_name', by.y = 'lower_final_gene_name', all.x = T)
neuro_analysis$input_and_papers <- merge(x = neuro_analysis$input_bm, y = neuro_analysis$pap_, by.x = 'external_gene_name', by.y = 'lower_final_gene_name', all.x = T)

readr::write_tsv(neuro_analysis$input_and_experiements , paste0(neuro_analysis$dir, '/neuro_and_experiements.tsv'))
readr::write_tsv(neuro_analysis$input_and_papers, paste0(neuro_analysis$dir, '/neuro_and_papers.tsv'))
### NEURO-NATURE COMPARISON ANALYSIS ###
...
### NEURO2 COMPARISON ANALYSIS ###
neuro2 <- list('dir' = 'neuro2')

neuro2$inputs <- read_excel_all_sheets(directory = neuro2$dir, file_name = 'neuro_2.xlsx', colNames_ = T)

neuro2$inputs <- purrr::map(
  .x = neuro2$inputs, 
  .f = function(input_){
    input_[[2]] <- as.numeric(input_[[2]])
    
    return(input_)
})

neuro2$input <- purrr::reduce(
  .x = neuro2$inputs, 
  .f = dplyr::full_join, 
  by = "symbol")

neuro2$input$symbol <- tolower(neuro2$input$symbol)

neuro2$input <-neuro2$input[!(neuro2$input$symbol == '#н/д'), ]

temp_biomart <- biomaRt::useMart("ENSEMBL_MART_ENSEMBL", dataset = "mmusculus_gene_ensembl")

neuro2$bm_results <- biomaRt::getBM(
  attributes = "external_gene_name",
  filters = "external_gene_name",
  values = neuro2$input$symbol,
  uniqueRows = T,
  mart = temp_biomart
)

neuro2$bm_results$symbol <- tolower(neuro2$bm_results$external_gene_name)

neuro2$input_bm <- merge(neuro2$input, neuro2$bm_results, by = 'symbol', all.x = T)

neuro2$missing <- neuro2$input_bm$symbol[is.na(neuro2$input_bm$external_gene_name)]


neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == '1110008f13rik'] <- 'Rab5if'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == '1110037f02rik'] <- 'Virma'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == '1190002n15rik'] <- 'Dipk2a'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == '1700020i14rik'] <- 'Oip5os1'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == '2010109i03rik'] <- 'Ly6m'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == '2310035c23rik'] <- 'Relch'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == '2410089e03rik'] <- 'Cplane1'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == '2700070h01rik'] <- '2700070h01rik'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == '2810428i15rik'] <- 'Rex1bd'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == '3110035e14rik'] <- 'Vxn'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == '3110039i08rik'] <- 'Mir100hg'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == '5031426d15rik'] <- '5031426d15rik'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'a230050p20rik'] <- 'Shfl'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'a330069e16rik'] <- 'a330069e16rik'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'a430078g23rik'] <- 'Arhgef18'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'a830010m20rik'] <- 'Btbd8'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'aes'] <- 'MIR548AE2'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'ai314180'] <- 'ai314180'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'akap2'] <- 'Pakap'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'bc030336'] <- 'Mosmo'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'bc037034'] <- 'Map11'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'bc039771'] <- 'bc039771'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'c77370'] <- 'Nexmif'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'd3ertd254e'] <- 'Zfp267'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'd830031n03rik'] <- 'd830031n03rik'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'dopey1'] <- 'Dop1a'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'fam178a'] <- 'Slf2'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'fam179b'] <- 'Togaram1'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'fam195b'] <- 'Mcrip1'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'fam19a1'] <- 'Tafa1'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'fam208b'] <- 'Tasor2'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'fam213b'] <- 'Prxl2b'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'fam35a'] <- 'Shld2'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'fam63b'] <- 'Mindy2'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'fam84a'] <- 'Lratd1'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'fam84b'] <- 'Lratd2'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'gm15800'] <- 'Hectd4'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'gm16702'] <- 'gm16702'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'h2afx'] <- 'H2ax'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'ipw'] <- 'Snhg14'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'kdelc2'] <- 'Poglut3'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'klhl17'] <- 'Plekhn1'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'march1'] <- 'Marchf1'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'march6'] <- 'Marchf6'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'march7'] <- 'Marchf7'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'march9'] <- 'Marchf9'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'mgea5'] <- 'Oga'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'mirc35hg'] <- 'mirc35hg'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'mkl2'] <- 'Mrtfb'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'palm2'] <- 'Pakap'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'tmem2'] <- 'Cemip2'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'tmem56'] <- 'Tlcd4'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'trove2'] <- 'Ro60'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'zbed6'] <- 'Gm38394'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'zcchc11'] <- 'Tut4'
neuro2$input_bm$external_gene_name[neuro2$input_bm$symbol == 'zcchc6'] <- 'Tut7'

neuro2$input_bm$gene_name_in_publication <- neuro2$input_bm$symbol
neuro2$input_bm$symbol <- NULL
neuro2$input_bm$external_gene_name <- tolower(neuro2$input_bm$external_gene_name)



neuro2$input_bm_pfc <- subset(x = neuro2$input_bm, select = !stringr::str_detect(string = colnames(neuro2$input_bm), pattern = '.*hp.*'))
neuro2$input_bm_hp <- subset(x = neuro2$input_bm, select = !stringr::str_detect(string = colnames(neuro2$input_bm), pattern = '.*pfc*'))



neuro2$input_bm_pfc_subsetter <- subset(x = neuro2$input_bm_pfc, select = stringr::str_detect(string = colnames(neuro2$input_bm_pfc), pattern = '.*pfc.*')) %>%
  apply(MARGIN = 1, FUN = function(row) {
    !all(is.na(row))
  })
neuro2$input_bm_hp_subsetter <- subset(x = neuro2$input_bm_hp, select = stringr::str_detect(string = colnames(neuro2$input_bm_hp), pattern = '.*hp.*')) %>%
  apply(MARGIN = 1, FUN = function(row) {
    !all(is.na(row))
  })

neuro2$input_bm_pfc <- subset(x = neuro2$input_bm_pfc, subset = neuro2$input_bm_pfc_subsetter)
neuro2$input_bm_hp <- subset(x = neuro2$input_bm_hp, subset = neuro2$input_bm_hp_subsetter)




neuro2$exp_ <- readr::read_tsv('exp_and_paper_numbers/exp_number_and_percentage_.tsv')
neuro2$pap_ <- readr::read_tsv('exp_and_paper_numbers/paper_number_.tsv')
neuro2$exp_pfc <- readr::read_tsv('exp_and_paper_numbers/exp_number_and_percentage_prefrontal_cortex.tsv')
neuro2$pap_pfc <- readr::read_tsv('exp_and_paper_numbers/paper_number_prefrontal_cortex.tsv')
neuro2$exp_hp <- readr::read_tsv('exp_and_paper_numbers/exp_number_and_percentage_hippocampus.tsv')
neuro2$pap_hp <- readr::read_tsv('exp_and_paper_numbers/paper_number_hippocampus.tsv')

neuro2$pap_$number_pap <- neuro2$pap_$number
neuro2$pap_pfc$number_pap <- neuro2$pap_pfc$number
neuro2$pap_hp$number_pap <- neuro2$pap_hp$number

neuro2$pap_$number <- NULL
neuro2$pap_pfc$number <- NULL
neuro2$pap_hp$number <- NULL

neuro2$exp_and_pap <- merge(x = neuro2$exp_, y = neuro2$pap_, by = 'lower_final_gene_name')
neuro2$exp_and_pap_pfc <- merge(x = neuro2$exp_pfc, y = neuro2$pap_pfc, by = 'lower_final_gene_name')
neuro2$exp_and_pap_hp <- merge(x = neuro2$exp_hp, y = neuro2$pap_hp, by = 'lower_final_gene_name')


neuro2$output <- merge(x = neuro2$input_bm, y = neuro2$exp_and_pap, by.x = 'external_gene_name', by.y = 'lower_final_gene_name', all.x = T)
neuro2$output_pfc <- merge(x = neuro2$input_bm_pfc, y = neuro2$exp_and_pap_pfc, by.x = 'external_gene_name', by.y = 'lower_final_gene_name', all.x = T)
neuro2$output_hp <- merge(x = neuro2$input_bm_hp, y = neuro2$exp_and_pap_hp, by.x = 'external_gene_name', by.y = 'lower_final_gene_name', all.x = T)


write.table(neuro2$output, paste0(neuro2$dir, '/neuro2_vs_all_output.tsv'), sep = '\t', dec = ',', row.names = F)
write.table(neuro2$output_pfc, paste0(neuro2$dir, '/neuro2_vs_pfc_output.tsv'), sep = '\t', dec = ',', row.names = F)
write.table(neuro2$output_hp, paste0(neuro2$dir, '/neuro2_vs_hp_output.tsv'), sep = '\t', dec = ',', row.names = F)
...
### NEURO3 COMPARISON ANALYSIS ###

neuro3 <- list('dir' = 'neuro3')

neuro3$input <- read.delim(file = paste0(neuro3$dir, '/neuro3.tsv'))

neuro3$input$gene <- tolower(neuro3$input$gene)

temp_biomart <- biomaRt::useMart("ENSEMBL_MART_ENSEMBL", dataset = "mmusculus_gene_ensembl")

neuro3$bm_results <- biomaRt::getBM(
  attributes = "external_gene_name",
  filters = "external_gene_name",
  values = neuro3$input$gene,
  uniqueRows = T,
  mart = temp_biomart
)

neuro3$bm_results$gene <- tolower(neuro3$bm_results$external_gene_name)

neuro3$input_bm <- merge(neuro3$input, neuro3$bm_results, by = 'gene', all.x = T)

neuro3$missing <- neuro3$input_bm$gene[is.na(neuro3$input_bm$external_gene_name)]

neuro3$input_bm$external_gene_name[neuro3$input_bm$gene == '1700024p16rik'] <- 'fyb2'
neuro3$input_bm$external_gene_name[neuro3$input_bm$gene == '4932418e24rik'] <- 'ccdc187'
neuro3$input_bm$external_gene_name[neuro3$input_bm$gene == 'pvrl3'] <- 'nectin3'

neuro3$input_bm$gene_name_in_publication <- neuro3$input_bm$gene
neuro3$input_bm$gene <- NULL
neuro3$input_bm$external_gene_name <- tolower(neuro3$input_bm$external_gene_name)

neuro3$exp_ <- readr::read_tsv('exp_and_paper_numbers/exp_number_and_percentage_.tsv')
neuro3$pap_ <- readr::read_tsv('exp_and_paper_numbers/paper_number_.tsv')
neuro3$exp_hp <- readr::read_tsv('exp_and_paper_numbers/exp_number_and_percentage_hippocampus.tsv')
neuro3$pap_hp <- readr::read_tsv('exp_and_paper_numbers/paper_number_hippocampus.tsv')

neuro3$pap_$number_pap <- neuro3$pap_$number
neuro3$pap_hp$number_pap <- neuro3$pap_hp$number

neuro3$pap_$number <- NULL
neuro3$pap_hp$number <- NULL

neuro3$exp_and_pap <- merge(x = neuro3$exp_, y = neuro3$pap_, by = 'lower_final_gene_name')
neuro3$exp_and_pap_hp <- merge(x = neuro3$exp_hp, y = neuro3$pap_hp, by = 'lower_final_gene_name')


neuro3$output <- merge(x = neuro3$input_bm, y = neuro3$exp_and_pap, by.x = 'external_gene_name', by.y = 'lower_final_gene_name', all.x = T)
neuro3$output_hp <- merge(x = neuro3$input_bm, y = neuro3$exp_and_pap_hp, by.x = 'external_gene_name', by.y = 'lower_final_gene_name', all.x = T)


write.table(neuro3$output, paste0(neuro3$dir, '/neuro3_vs_all_output.tsv'), sep = '\t', dec = ',', row.names = F)
write.table(neuro3$output_hp, paste0(neuro3$dir, '/neuro3_vs_hp_output.tsv'), sep = '\t', dec = ',', row.names = F)
